...
test:group("getBounds()", function ()
  -- We use rectangles here because the Shape class has no bounds to test.
  ...
  test:run("should include outer borders", function ()
    local rect = Rectangle.new(100, 200):setBorderWidth(10)
    test:assertEqual({-10, -10, 120, 220}, {rect:getBounds()})
  end)
  test:run("should not include outer border smoothing", function ()
    local rect = Rectangle.new(100, 200):setBorderWidth(10):setBorderSmoothing(5)
    test:assertEqual({-10, -10, 120, 220}, {rect:getBounds()})
  end)
